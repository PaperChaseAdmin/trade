...
MAX_WORKERS       = 1   # concurrent bots — single worker to avoid OpenRouter rate limits
...
def get_prices(tickers):
    """Fetch prices with yfinance, faster with fewer API calls."""
    prices = {}
    try:
        ...
        if not data.empty:
            ...
            if len(tickers) == 1:
                ...
                for t in tickers:
                    try:
                        prices[t] = round(float(close[t].dropna().iloc[-1]), 4)
                    except:
                        pass
        else:
            raise ValueError("empty data from yfinance batch")
    except Exception as e:
...
def get_market_data():
    try:
        return requests.get(MARKET_SENTINEL, timeout=15).json()
    except Exception as e:
...
def pf_path(b):
    return f"{DATA_DIR}/{b}/portfolio.json"
def tr_path(b):
    return f"{DATA_DIR}/{b}/trades.json"
...
def sync_prices(pf, prices):
    for t in pf["positions"]:
        if t in prices:
            pf["positions"][t]["current_price"] = prices[t]
...
# ── Gemini Decision ───────────────────────────────────────────────────────────
def get_gemini_decision(bot_id, profile, pf, prices, changes, market_data):
    """Returns (decision_dict, context_dict). Uses Gemini 2.0 Flash."""
...
def main():
    ...
    for pid, p in BOT_PROFILES.items():
        ...
        try:
            all_tickers.update(load_json(pf_path(pid)).get("positions", {}).keys())
        except:
            pass
    ...
    print(f"\n[3] Running {len(BOT_PROFILES)} bots (max {MAX_WORKERS} concurrent)...")
    results = [None] * len(BOT_PROFILES)
    bot_items = list(BOT_PROFILES.items())

    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        future_map = {
            executor.submit(run_bot, bot_id, prices, changes, md): (idx, bot_id, profile)
            for idx, (bot_id, profile) in enumerate(bot_items)
        }
        for future in as_completed(future_map):
            idx, bot_id, profile = future_map[future]
            try:
                pf = future.result()
                results[idx] = (pf, profile)
            except Exception as e:
                print(f"\n  FAIL {bot_id}: {e}")
                results[idx] = None
